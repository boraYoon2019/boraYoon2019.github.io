...
axios
  ...
  .then((response) => {
    ...
    const projectWeb = projects.web
      .map((project) => create_projectItem(project, "web"))
      .join("");
    const projectApp = projects.app
      .map((project) => create_projectItem(project, "app"))
      .join("");
    ...
    for (const exp of experience) {
      const experienceItem = make_element_withClassesArray("li", [
        "base-spacing",
        "bottom-spacing",
      ]);
      ...
      if (exp.link !== "") {
        ...
        experienceLink.setAttribute("target", "_blank");
        experienceLink.textContent = "(" + exp.link + ")";
        experienceItem.appendChild(experienceLink);
      }

      const experiencePosition = make_element_withClassesArray("h4", [
        "align-right",
      ]);
      ...
      const experienceDuration = make_element_withClassesArray("p", [
        "align-right",
        "text-bolder",
      ]);
      ...
      const experienceContent = make_element_withClassesArray("p", [
    ...
    for (const item of education) {
      const educationItem = make_element_withClassesArray("li", [
        "base-spacing",
        "bottom-spacing",
      ]);
      ...
      const educationDuration = make_element_withClassesArray("p", [
        "align-right",
        "text-bolder",
      ]);
      ...
      const educationContent = make_element_withClassesArray("p", [
        "updown-double-spacing",
      ]);
