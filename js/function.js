		...
		(() => {
			if (skills[key] !== '' || skills[key] !== undefined) {
				isInSkillSection === true
					? (skillContainer = append_skillList_inSection(
							skillContainer,
							skills[key],
							key
					  ))
					: (skillContainer = append_skillList(
							skillContainer,
							skills[key],
							key
					  ));
...
function append_skillItems(skillList, skillArray, type) {
	...
	for (const skill of skillArray) {
		...
		skillItem.classList.add(
			type === 'skill' ? 'skills--general' : 'language--general'
		);
...
function append_skillList_inSection(skillContainer, skillArray, type) {
	...
	const skillList = make_element_withClassesArray('ul', [
		'content__description',
		type,
	]);

	if (skillArray !== '') {
		finalSkillContainer.appendChild(
			append_skillItems_InSection(skillList, skillArray, type)
		);
	}
...
function skillItems_onClick(event) {
	...
	const targetProjectItems = document.querySelectorAll(
...
function create_projectItem(project, tagClass) {
	...
	return `
  <div class="projects__item ${tagClass} ${skills_class}" data-head="${
		project.head
	}" data-kind="${tagClass}">
    <img src="${
			project.thumbnail || './image.png'
		}" class="img-responsive" alt="프로젝트 이미지" data-head="${
		project.head
	}" data-kind="${tagClass}">
    <div class="projects-caption" data-head="${
			project.head
		}" data-kind="${tagClass}">
      <h4 class="projects-head" data-head="${
				project.head
			}" data-kind="${tagClass}">${project.head}</h4>
      <p class="projects-feature" data-head="${
				project.head
			}" data-kind="${tagClass}">${project.feature}</p>
      <p class="projects-feature" data-head="${
				project.head
			}" data-kind="${tagClass}"><b>${project.duration}</b></p>
      <p class="projects-subhead base-spacing" data-head="${
				project.head
			}" data-kind="${tagClass}">${project.subhead}</p>
      <p class="projects-skills" data-head="${
				project.head
			}" data-kind="${tagClass}">${skills_text}</p>
    </div>
  </div>`;
}
...
function highlight_navMenu() {
	...
	sidebar_items.forEach((item, index) => {
		...
		const compare =
			activeSection.offsetTop + 100 <= scroll_pos &&
			activeSection.offsetTop + 50 + (activeSection.offsetHeight + 50) >
				scroll_pos;
...
function onProjectItemClick(event, projects_array) {
	...
	const clickedProject = projects_array[projectClass].find(
		(project) => project.head === projectName
	);
...
function modalSetting(clickedProject) {
	...
	const modal__content__layout = document.querySelector(
		'.modal__content__layout'
	);

	// 모달 만들기
	const modal__content = make_element_withClassesArray('div', [
		'modal__content',
	]);
	modal__content__layout.appendChild(modal__content);

	// 제목
	const head = make_element_withClassesArray('h1', [
		'modal-head',
		'base-spacing',
	]);
	...
	const modalSubheadBox = make_element_withClassesArray('div', [
		'modal-subhead',
	]);
	...
	// 메인 슬라이드 컨테이너 태그 생성
	const slidesContainer = make_element_withClassesArray('div', [
		'slides__container',
	]);
	...
	if (imgs[0] !== '') modal__content.appendChild(slideThumbnails);

	// skillList
	const skillsList = make_element_withClassesArray('div', [
		'modal__skills-list',
		'double-spacing',
	]);
	...
	// 깃헙, 사이트 링크 anchor
	const moreBox = make_element_withClassesArray('div', [
		'link__item',
		'row-flex',
	]);
	...
	const description = make_element_withClassesArray('div', [
		'modal__description',
		'double-spacing',
	]);
	...
	description.textContent =
		typeof clickedProject.description === 'string'
			? clickedProject.description
			: clickedProject.description.join('\n');
